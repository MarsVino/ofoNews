<template>
    <div class="homeIndex-container" ref="betterScroll">
        <div>
            ...
            <div v-show="newsRefresh" class="topRefresh">
                <img v-lazy="refreshPic">
            </div>
...
export default {
  ...
  created() {
      ...
      console.log(this.envLink)
      const code = window.location.search.split('&')[0].split('=')[1]
      console.log(code)
      window.ofoResponseProxy('setTitle',['看看']);
      this.getDataList()
  },

  mounted(){
      console.log(12243,Utils.getHashParams())
      this.$nextTick(() => {
          this._initScroll()
      })
      //window.OFO_ENV.ofoApp('','/17312/ofo-local-17312.apk')
  },
    ...
    _initScroll () {
          if (!this.scroll) {
              ...
              this.scroll.on('scroll', (pos) => {
                  let dthat = this
                  if (pos.y > 50 && !this.newsRefresh) {
                      dthat.newsRefresh = true
                      dthat.curPage = 1
                      dthat.curType = 'top_refresh'
                      console.log("顶部刷新")
                      this.getDataList()
              ...
              this.scroll.on('touchEnd', (pos) => {
                  ...
                  if (Math.abs(pos.y) >= scrollTop + 50) {
                      //到底了
                      this.curPage++
/*                      if (this.curType == 'top_refresh'){
                          this.curPage = 1
                      }*/
                      this.curType = 'bottom_refresh'
                      this.getDataList()
                      this.delayInitScroll()
                  }
              })
    ...
    * */
    ...
    getDataList(){
        ...
        let params = {
            ...
            refresh_type: this.curType
        }
       API.newsList(params)
       .then(data => {
           if (dthat.newsRefresh) {
               dthat.dataList = []
           }
           dthat.refreshText = data.data.kankan_list.top_msg
           dthat.dataList = dthat.dataList.concat(data.data.kankan_list.news_list)
           dthat.newsRefresh = false
