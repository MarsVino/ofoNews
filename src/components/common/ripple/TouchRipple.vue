<template>
  ...
  <div @mousedown="handleMouseDown" @mouseup="end()" @mouseleave="end()" @touchstart="handleTouchStart" @touchend="end()" @touchcancel="end()">
    <!--外层包裹防止波纹溢出-->
    <div class="ripple-wrap" :style="styleObj" ref="holder">
      <!--多个波纹用 v-for 控制-->
    ...
    methods: {
        ...
        handleTouchMove(event) {
            ...
            const deltaY = Math.abs(
                event.touches[0].clientY - this.firstTouchY
            );
            const deltaX = Math.abs(
                event.touches[0].clientX - this.firstTouchX
            );
            // 滑动范围在 > 6px 结束波纹点击效果
...
.ripple-wrap {
    ...
    transform: translate3d(0, 0, 0) rotate(0deg); /*父元素使用transform:translate3d 属性*/
